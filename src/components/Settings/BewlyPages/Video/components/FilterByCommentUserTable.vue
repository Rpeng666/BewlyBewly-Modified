...
function handleAddFilter() {
  ...
  const hasDuplicate = settings.value.filterByCommentUser.find(
  ...
  settings.value.filterByCommentUser.unshift({ ...addingFilter.value })
  nextTick(() => {
...
async function handleEditFilter(index: number, focusItem: 'keyword' | 'remark' = 'keyword') {
  ...
  editingFilter.value = { ...settings.value.filterByCommentUser[index] }
  await nextTick()
...
function handleConfirmFilter(index: number) {
  ...
  settings.value.filterByCommentUser[index] = { ...editingFilter.value }
  if (index !== -1)
    editingIndex.value = -1
}

function handleDeleteFilter(index: number) {
  settings.value.filterByCommentUser.splice(index, 1)
}
...
<template>
  <div>
    ...
    <div
      v-if="settings.filterByCommentUser.length > 0"
      grid="~ cols-[1fr_1fr_80px] gap-2"
      bg="$bew-fill-1" p-2 rounded="$bew-radius"
    >
      <div text-center font-bold>
        {{ $t('settings.filter_by_user_table.username_or_mid') }}
      </div>
      <div text-center font-bold>
    ...
    >
      <div
        v-for="(item, index) in settings.filterByCommentUser"
        :key="index"
        grid="~ cols-[1fr_1fr_80px] gap-2"
        items-center
