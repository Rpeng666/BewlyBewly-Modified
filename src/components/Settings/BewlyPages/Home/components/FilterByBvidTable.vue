...
function handleAddFilter() {
  ...
  const hasDuplicate = settings.value.filterByBvid.find(
    (item, itemIndex) => item.keyword === addingFilter.value.keyword.trim() && itemIndex !== editingIndex.value,
  )
  if (hasDuplicate) {
    toast.warning('This BV ID filter already exist!!!')
    return
  }
  settings.value.filterByBvid.unshift({ ...addingFilter.value })
  nextTick(() => {
...
async function handleEditFilter(index: number, focusItem: 'keyword' | 'remark' = 'keyword') {
  ...
  editingFilter.value = { ...settings.value.filterByBvid[index] }
  await nextTick()
...
function handleConfirmFilter(index: number) {
  ...
  settings.value.filterByBvid[index] = { ...editingFilter.value }
  if (index !== -1)
    editingIndex.value = -1
}

function handleDeleteFilter(index: number) {
  settings.value.filterByBvid.splice(index, 1)
}
...
<template>
  <div>
    ...
    >
      ...
      <ListItem
        v-for="(item, index) in settings.filterByBvid" :key="item.keyword"
        :style="{
