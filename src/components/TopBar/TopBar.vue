...
<template>
  <Transition name="top-bar">
    <header v-if="showTopBar" ref="headerTarget" w="full" transition="all 300 ease-in-out"
      :class="{ 'hide': hideTopBar, 'force-white-icon': forceWhiteIcon }"
      :style="{ position: isTopBarFixed ? 'fixed' : 'absolute' }">
      <main max-w="$bew-page-max-width" flex="~ justify-between items-center gap-4" p="x-12" m-auto
        h="$bew-top-bar-height">
        <!-- Top bar mask -->
        <div v-if="!reachTop" style="
          ...
          pos="absolute top-0 left-0" w-full h="[calc(var(--bew-top-bar-height)+16px)]" pointer-events-none
          transform-gpu />

        <div pos="absolute top-0 left-0" w-full pointer-events-none opacity-100 duration-300 :style="{
          background: `linear-gradient(to bottom, ${forceWhiteIcon
        ...
        <Transition name="fade">
          <div v-if="settings.showTopBarThemeColorGradient && !forceWhiteIcon && reachTop && isDark"
        ...
        <div shrink-0 flex="inline xl:1 justify-start items-center gap-2" pos="relative" z-1>
          <div ref="channels" z-1 relative w-fit>
            <a ref="logo" href="//www.bilibili.com" target="_top" class="group logo" :class="{
              activated: popupVisible.channels,
            }" grid="~ place-items-center" rounded="46px" duration-300 w-46px h-46px transform-gpu
              bg="hover:$bew-theme-color" @click="event => handleClickTopBarItem(event, 'channels')">
              <svg t="1720198072316" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="1477" width="36" height="36" :style="{
                ...
                }" group-hover:fill="!white" group-hover:filter="!none" duration-300 mt--1px>
                <path
            ...
            <Transition name="slide-in">
              <ChannelsPop v-if="popupVisible.channels" class="bew-popover" pos="!left-0 !top-50px"
        ...
        <div flex="inline 1 md:justify-center items-center" w="full">
          <Transition name="slide-out">
            <SearchBar v-if="showSearchBar" class="search-bar" :style="{
        ...
        <div class="right-side" flex="inline xl:1 justify-end items-center">
          <div class="others" flex="~ items-center gap-1" h-46px px-5px text="$bew-text-1" transform-gpu>
            <div v-if="!isLogin" class="right-side-item" important-w-auto>
            ...
            <template v-if="isLogin">
              ...
              <div class="hidden lg:flex" gap-1>
                <!-- Moments -->
                <div ref="moments" class="right-side-item" :class="{ active: popupVisible.moments }"
                  @click="event => handleClickTopBarItem(event, 'moments')">
                  <template v-if="newMomentsCount > 0">
                    <div v-if="settings.topBarIconBadges === 'number'" class="unread-num-dot">
                      {{ newMomentsCount > 99 ? '99+' : newMomentsCount }}
                    </div>
                    <div v-else-if="settings.topBarIconBadges === 'dot'" class="unread-dot" />
                  </template>
                  <ALink :class="{ 'white-icon': forceWhiteIcon }" href="https://t.bilibili.com"
                    :title="$t('topbar.moments')" type="topBar">
                    <div i-tabler:windmill />
                  </ALink>

                  <Transition name="slide-in">
                    <MomentsPop v-show="popupVisible.moments" ref="momentsTransformer" class="bew-popover"
                      @click.stop="() => { }" />
                  </Transition>
                </div>

                <!-- Favorites -->
                <div ref="favorites" class="right-side-item" :class="{ active: popupVisible.favorites }"
                  @click="event => handleClickTopBarItem(event, 'favorites')">
                  <ALink :class="{ 'white-icon': forceWhiteIcon }" :href="`https://space.bilibili.com/${mid}/favlist`"
                    :title="$t('topbar.favorites')" type="topBar">
                    <div i-mingcute:star-line />
                  </ALink>

                  <Transition name="slide-in">
                    <KeepAlive>
                      <FavoritesPop v-if="popupVisible.favorites" ref="favoritesTransformer" class="bew-popover"
                        @click.stop="() => { }" />
                    </KeepAlive>
                  </Transition>
                </div>
                ...
                <!-- History -->
                <div ref="history" class="right-side-item" :class="{ active: popupVisible.history }"
                  @click="event => handleClickTopBarItem(event, 'history')">
                  <ALink :class="{ 'white-icon': forceWhiteIcon }" href="https://www.bilibili.com/history"
                    :title="$t('topbar.history')" type="topBar">
                    <div i-mingcute:time-line />
                  </ALink>

                  <Transition name="slide-in">
                    <HistoryPop v-if="popupVisible.history" ref="historyTransformer" class="bew-popover"
                      @click.stop="() => { }" />
                  </Transition>
                </div>

                <!-- Watch later -->
                <div ref="watchLater" class="right-side-item" :class="{ active: popupVisible.watchLater }"
                  @click="event => handleClickTopBarItem(event, 'watchLater')">
                  <ALink :class="{ 'white-icon': forceWhiteIcon }" href="https://www.bilibili.com/watchlater/list"
                    :title="$t('topbar.watch_later')" type="topBar">
                    <div i-mingcute:carplay-line />
                  </ALink>

                  <Transition name="slide-in">
                    <WatchLaterPop v-if="popupVisible.watchLater" ref="watchLaterTransformer" class="bew-popover"
                      @click.stop="() => { }" />
                  </Transition>
                </div>
                ...
                <div class="right-side-item">
                  <a :class="{ 'white-icon': forceWhiteIcon }" href="https://member.bilibili.com/platform/home"
                    target="_blank" :title="$t('topbar.creative_center')">
              ...
              <!-- More -->
              <div ref="more" class="right-side-item lg:!hidden flex" :class="{ active: popupVisible.more }"
                @click="event => handleClickTopBarItem(event, 'more')">
                <a :class="{ 'white-icon': forceWhiteIcon }" title="More">
                  <div i-mingcute:menu-line />
                </a>

                <Transition name="slide-in">
                  <MorePop v-show="popupVisible.more" ref="moreTransformer" class="bew-popover"
                    @click.stop="() => { }" />
                </Transition>
              </div>
              ...
              <div class="hidden lg:flex" gap-1 items-center>
                <!-- Divider -->
                <div :class="{ 'white-icon': forceWhiteIcon }" w-2px h-16px bg="$bew-border-color" mx-1 rounded-4px />

                <!-- Upload -->
                <div ref="upload" class="right-side-item" :class="{ active: popupVisible.upload }"
                  @click="event => handleClickTopBarItem(event, 'upload')">
                  <a class="upload" :class="{ 'white-icon': forceWhiteIcon }"
                    style="backdrop-filter: var(--bew-filter-glass-1);"
                    href="https://member.bilibili.com/platform/upload/video/frame" target="_blank"
                    :title="$t('topbar.upload')">
                    <div i-mingcute:upload-line flex-shrink-0 />
                  </a>

                  <Transition name="slide-in">
                    <UploadPop v-if="popupVisible.upload" ref="uploadTransformer" class="bew-popover"
                      @click.stop="() => { }" />
                  </Transition>
                </div>

                <!-- Notifications -->
                <div ref="notifications" class="right-side-item" :class="{ active: popupVisible.notifications }"
                  ...
                  <Transition name="slide-in">
                    <NotificationsPop v-if="popupVisible.notifications" ref="notificationsTransformer"
                      class="bew-popover" @click.stop="() => { }" @item-click="handleNotificationsItemClick" />
                  </Transition>
                </div>
              </div>
            </template>
            ...
            <!-- Avatar -->
            <div v-if="isLogin" ref="avatar" :class="{ hover: popupVisible.userPanel }" class="avatar right-side-item"
              @click="event => handleClickTopBarItem(event, 'userPanel')">
              <ALink ref="avatarImg" :href="`https://space.bilibili.com/${mid}`" type="topBar" class="avatar-img"
                :class="{ hover: popupVisible.userPanel }" :style="{
                  backgroundImage: `url(${`${userInfo.face}`.replace(
                    'http:',
                    '',
                  )})`,
                }" />
              <div ref="avatarShadow" class="avatar-shadow" :class="{ hover: popupVisible.userPanel }" :style="{
                backgroundImage: `url(${`${userInfo.face}`.replace(
                  'http:',
                  '',
                )})`,
              }" />
              <svg v-if="userInfo.vip?.status === 1" class="vip-img" :class="{ hover: popupVisible.userPanel }"
                ...
                w="28%" h="28%" z-1 pos="absolute bottom--20px right-28px" duration-300 />

              <Transition name="slide-in">
                <UserPanelPop v-if="popupVisible.userPanel" ref="avatarTransformer" :user-info="userInfo" after:h="!0"
                  class="bew-popover" pos="!left-auto !right-0" transform="!translate-x-0" @click.stop="() => { }" />
              </Transition>
            </div>
          </div>
        </div>
      ...
      <KeepAlive v-if="settings.openNotificationsPageAsDrawer">
        <NotificationsDrawer v-if="drawerVisible.notifications" :url="notificationsDrawerUrl"
